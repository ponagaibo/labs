...
void find_tree(tree root, tree* r)
{
...
void search_tree (tree t, tree *r)
{
    extern st;
    st=0;
    printf("in search\n");
    if (t->right!=NULL) search_tree(t->right, r);
    if (t->left!=NULL) search_tree(t->left, r);
    if (t->val_tree.type_t==0 && (t->right==NULL || t->left==NULL) && (st==0))
    {
        r=&t;
        st++;
        //printf("st=%d\n",st);
...
void print_tree(tree root,int* i)
{
    //extern tree_size;
    //tree_size=1;
    if((root->val_tree.type_t==0)||(root->val_tree.type_t==2)) printf("in tree at %c\n",root->val_tree.symb_t);
    else if(root->val_tree.type_t==1) printf("in tree at %d\n",root->val_tree.value_t);
    (*i)++;
    //tree_size++;
...
int main()
{
    ...
    //printf("\nsize of ress is %d\n",count);
    tree T=NULL;
    tree t2=NULL, t3=NULL;
    tree* t1;
    //if (T) T = remove_tree(T);
    T=malloc(sizeof(Node_t));
    T->right=NULL;
    T->left=NULL;
    T->val_tree.value_t=ress[count-1].value;
    T->val_tree.type_t=ress[count-1].type;
    T->val_tree.symb_t=ress[count-1].symb;
    //t3=malloc(sizeof(Node_t));
