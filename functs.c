...
int check_expr(char* str)
{
    ...
    for(i=0;i<count;i++){
      if( str[i]<=39 || str[i]==44 || str[i]==46 || (str[i]>=58&&str[i]<=64) || (str[i]>=91&&str[i]<=93) || str[i]==95 || str[i]==96 || (str[i]>=123&&str[i]<=127) )
      {
        printf("wrong -1\n");
        return 0;
      }
        if(str[i]=='(')s++;
        if(str[i]==')')s--;
        if(((str[i]>=48&&str[i]<=57)||is_var(str[i]))&&(str[i+1]=='(')){
            printf("wrong 1\n");
            return 0;
        }
